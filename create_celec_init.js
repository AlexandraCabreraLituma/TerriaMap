...
console.log("TERRIA_URL=", process.env.TERRIA_URL);
...
fs.readFile("./wwwroot/init/simple.json", "utf8", function (err, data) {
  ...
  result = result.replace(/\$TERRIA_URL/g, process.env.TERRIA_URL);

  fs.writeFile("./wwwroot/init/celec.json", result, "utf8", function (err) {
    if (err) return console.log(err);
    console.log(
      "CELEC configuration file ./wwwroot/init/celec.json created successful 🚀🚀🚀"
    );
  });
});
fs.readFile("./wwwroot/init/simple_energia.json", "utf8", function (err, data) {
  ...
  fs.writeFile(
    "./wwwroot/init/celec_energia.json",
    result,
    "utf8",
    function (err) {
      if (err) return console.log(err);
      console.log(
        "CELEC configuration file ./wwwroot/init/celec_energia.json created successful 🚀🚀🚀"
      );
    }
  );
});
fs.readFile("./wwwroot/init/simple_geo.json", "utf8", function (err, data) {
  ...
  fs.writeFile("./wwwroot/init/celec_geo.json", result, "utf8", function (err) {
    if (err) return console.log(err);
    console.log(
      "CELEC configuration file ./wwwroot/init/celec_geo.json created successful 🚀🚀🚀"
    );
  });
});
fs.readFile(
  "./wwwroot/init/simple_hidrometeorologia.json",
  "utf8",
  function (err, data) {
    if (err) {
      return console.log(err);
    ...
    fs.writeFile(
      "./wwwroot/init/celec_hidrometeorologia.json",
      result,
      "utf8",
      function (err) {
        if (err) return console.log(err);
        console.log(
          "CELEC configuration file ./wwwroot/init/celec_hidrometeorologia.json created successful 🚀🚀🚀"
        );
      }
    );
  }
